...
def modify_sql(sql_template, itemlistlist, classifications):
    ...
    for classification in classifications:
        for item_zh,item_english, item_ID in itemlistlist[classification]:
            statement = f""",
    AVG(CASE WHEN npe.ExaminationItemID = '{item_ID}' THEN npe.Result ELSE NULL END) AS {item_english}_Avg,
    MAX(CASE WHEN npe.ExaminationItemID = '{item_ID}' THEN npe.Result ELSE NULL END) AS {item_english}_Max,
    MIN(CASE WHEN npe.ExaminationItemID = '{item_ID}' THEN npe.Result ELSE NULL END) AS {item_english}_Min"""
...
with open(f'sql/dataforxgboost_{current_date}.sql', 'w') as f:
